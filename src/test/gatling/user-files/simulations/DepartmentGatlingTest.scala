...
class DepartmentGatlingTest extends Simulation {
    ...
    val authorization_header = "Basic " + Base64.getEncoder.encodeToString("departmentapp:my-secret-token-to-change-in-production".getBytes(StandardCharsets.UTF_8))
    ...
    val scn = scenario("Test the Department entity")
        ...
        .formParam("client_id", "departmentapp")
        ...
        .repeat(2) {
            ...
            .post("/department/api/departments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "city":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_department_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created department")
                .get("/department${new_department_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created department")
            .delete("/department${new_department_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
